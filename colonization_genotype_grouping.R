...
# written by Jonathan Schnabel 20th October 2017
# email: jonathan.schnabel31@gmail.com

# the script generates a grouped (by genotype) plot from grid intersect quantification of trypan blue staining 
# arbuscular mycorrhizal fungus colonization of roots
...
{
  begin_group = c(begin_group, i * (1 + length(struct)) + 1.5)
}
...
#activate the following three lines to group data by genotype
matri = matrix(medians, ncol = length(genotypes), byrow = T)
colnames(matri) = genotypes
rownames(matri) = struct
...
h = barplot(matri, beside=T, ylim = c(0,100), ylab = 'Total root length colonization (%)', 
            col = cols[1:length(struct)], names.arg = unique(quantif[,1]), border = 'white')

#disables clipping and draws a legend
par(xpd=T)
legend(x = begin_group[length(genotypes)] + length(struct) - 1.5, y = 120,legend = struct, fill = cols, bty='n')
